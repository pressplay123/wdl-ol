...
class IGraphicsMac : public IGraphics
{
  public:
    ...
    void PromptForFile(WDL_String *pFilename, EFileAction action = kFileOpen, WDL_String *pDir = 0,
    ...
    bool OpenURL(
        const char *url, const char *msgWindowTitle = 0, const char *confirmMsg = 0, const char *errMsgOnFailure = 0);
...
inline CFStringRef MakeCFString(const char *cStr) { return CFStringCreateWithCString(0, cStr, kCFStringEncodingUTF8); }
...
inline int AdjustFontSize(int size) { return int(0.9 * (double)size); }
